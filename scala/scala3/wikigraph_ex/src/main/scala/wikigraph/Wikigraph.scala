...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    val ls = client.linksFrom(of).flatMap(x => WikiResult.traverse(x.toSeq)(client.nameOfArticle))
    ls.map(x => x.toSet)
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      println(s"maxDepth $maxDepth visited $visited")
      if (q.isEmpty){ // process till q is empty or max depth or found target
        WikiResult.successful(None)
      }
      else {
        val ((depth,artid),qnew) = q.dequeue // get first element out of the q
        if (depth >= maxDepth) {
          WikiResult.successful(None) // process till max depth
        }
        else          
        if (visited.contains(artid))
        {
            iter(visited,qnew) // do not add to give just process next
        }
        else {
          val links = client.linksFrom(artid)
          val result = links.flatMap(ls => {
            if (ls.contains(target)){
              WikiResult.successful(Some(depth)) // found target
            }
            else {
              val newDepthArt = ls.map(aid => (depth+1,aid))
              iter(visited + artid,qnew.enqueue(newDepthArt))
            }
          })
          result.fallbackTo(iter(visited + artid,qnew))
        }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    ...
    val titleFromToPair:Seq[(String,String)] = 
      for {
        fromTitle <- titles
        toTitle <- titles
        if fromTitle != toTitle
      } yield (fromTitle,toTitle)

    WikiResult.traverse(titleFromToPair) {  titlePairs =>
      val (from,to) = titlePairs
      for
        artPair <- client.searchId(from).zip(client.searchId(to))
        (fromid,toid) = artPair
        distance <- breadthFirstSearch(fromid,toid,maxDepth)
        found <- WikiResult.successful(from,to,distance)
      yield found
    }
