...
object `collection.worksheet` {
def args = `collection.worksheet_sc`.args$
...
var futures: List[Future[List[Try[Int]]]] = files1.map(processFile)
...
object `collection.worksheet_sc` {
  ...
  def main(args: Array[String]): Unit = {
    args$set(args)
    `collection.worksheet`.hashCode() // hasCode to clear scalac warning about pure expression in statement position
  }
}
