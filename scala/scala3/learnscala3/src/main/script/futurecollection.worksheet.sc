...
def performComputation(value: Int): Future[Int] = Future {
  Thread.sleep(1000) // Simulate some async computation
  value * 2
}
...
val futures = values.map(performComputation)

// Collect results into a single future
val allCompleted: Future[List[Int]] = Future.sequence(futures)

// Perform a function after all futures are completed
allCompleted.foreach { results =>
